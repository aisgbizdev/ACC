...
import {
  ...
  User, UserCircle, FileBarChart, Users, Settings, LogOut, ChevronDown,
} from "lucide-react";
...
const NAV_ITEMS = [
  { href: "/dashboard",     label: "Dashboard",      icon: LayoutDashboard, roles: ["apuppt", "dk", "du", "owner", "superadmin"] },
  {
    ...
    roles: ["apuppt"],
    children: [
      { href: "/activity/daily", label: "Daily" },
      { href: "/activity/monthly", label: "Monthly" },
      { href: "/activity/quarterly", label: "Triwulan" },
    ],
  },
...
export function Navbar() {
  ...
  const [activityOpen, setActivityOpen] = useState(() => location.startsWith("/activity/"));

  if (!user) return null;

  const visibleItems = NAV_ITEMS.filter((item) => item.roles.includes(user.role));
  const isOnActivityRoute = location.startsWith("/activity/");
      ...
      <nav className="flex-1 overflow-y-auto py-2 scrollbar-thin">
        {visibleItems.map((item) => {
          ...
          if (item.children && item.children.length > 0) {
            ...
            const isOpen = activityOpen || isChildActive;
            ...
            return (
              <div key={item.label} className="mb-0.5">
                <button
                  ...
                  className={`w-[calc(100%-1rem)] flex items-center gap-3 px-4 py-2.5 mx-2 rounded-lg text-sm transition-colors ${
                    isChildActive
                ...
                >
                  ...
                  <span className="text-xs leading-tight flex-1 text-left">{item.label}</span>
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isOpen ? "rotate-180" : ""}`} />
                </button>

                {isOpen && (
                  <div className="mt-1 ml-8 mr-2 space-y-0.5">
                    {item.children.map((child) => {
                      const isChildItemActive = location === child.href || location.startsWith(child.href + "/");
                      return (
                        <Link
                          key={child.href}
                          href={child.href}
